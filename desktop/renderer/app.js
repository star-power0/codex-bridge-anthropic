...
document.addEventListener("click", (e) => {
  const btn = e.target.closest("[data-context-preset]");
  if (!btn) return;
  const value = btn.dataset.contextPreset;
  const container = btn.closest("label, .model-context-inline, .capability-fields, fieldset, form");
...
document.addEventListener("change", (e) => {
  const select = e.target.closest("[data-context-preset-select]");
  if (!select || !select.value) return;
  const value = select.value;
  const container = select.closest(".model-context-inline, label, fieldset, form");
  const input = container?.querySelector("input[type=number]");
  if (input) {
    input.value = value;
    input.dispatchEvent(new Event("input", { bubbles: true }));
...
function renderProviderEditor() {
  ...
  const refreshButton = provider.baseUrl && provider.authMode !== "codex_openai"
    ...
    : "";
  const removeButton = provider.custom || provider.id.startsWith("custom-")
    ? `<button class="ghost-button light small danger" type="button" data-remove-provider="${escapeHtml(provider.id)}">删除供应商</button>`
    : "";
      ...
      <div class="provider-settings-grid" data-provider-settings data-provider-id="${escapeHtml(provider.id)}" data-provider-key-env="${escapeHtml(provider.keyEnv || "")}" data-provider-auth-mode="${escapeHtml(provider.authMode || "api_key")}" data-provider-custom="${provider.custom ? "true" : "false"}">
        ...
        <label>
          ...
          <select data-provider-api>
            ...
            <option value="anthropic_messages" ${apiValue === "anthropic_messages" ? "selected" : ""}>Anthropic Messages</option>
          </select>
        </label>
        <label>
          <span>获取 Key 链接</span>
...
function bindProviderEditorActions(root) {
  ...
  root.querySelectorAll("[data-remove-provider]").forEach((button) => {
    button.addEventListener("click", () => removeProvider(button));
  });
...
function removeProvider(button) {
  return runAction(button, async () => {
    const providerId = button.dataset.removeProvider || button.closest(".provider-editor-card")?.dataset.providerId || "";
    const provider = providerFor(providerId);
    const providerName = provider?.name || providerId;
    if (!confirm(`确定要删除供应商“${providerName}”及其旗下的所有自定义模型吗？`)) {
      return;
    }
    ...
    draftSelection = [...state.selectedModelIds];
    openModelCatalog();
    showToast(`供应商“${providerName}”及旗下模型已成功删除。`);
  });
}
...
function modelCatalogSummary(model) {
  ...
  const modalities = new Set(inputModalitiesForModel(model));
  const parts = [apiLabelShort(model.api)];
...
function contextPresetButtonsHtml() {
  return `
    <div class="context-preset-bar" aria-label="上下文快捷选项">
      <button type="button" class="ghost-button light small" data-context-preset="65536">64K</button>
      <button type="button" class="ghost-button light small" data-context-preset="128000">128K</button>
      <button type="button" class="ghost-button light small" data-context-preset="258400">256K</button>
      <button type="button" class="ghost-button light small" data-context-preset="500000">500K</button>
      <button type="button" class="ghost-button light small" data-context-preset="1000000">1M</button>
      <button type="button" class="ghost-button light small" data-context-preset="2000000">2M</button>
    </div>
  `;
}
...
function inlineModelContextControl(model) {
  return `
    <div class="model-context-inline" data-model-context="${escapeHtml(model.presetId)}">
      ...
      <select class="context-preset-select" data-context-preset-select title="快捷选择上下文大小">
        <option value="" disabled selected>预设 ▾</option>
        <option value="65536">64K</option>
        <option value="128000">128K</option>
        <option value="258400">256K</option>
...
function apiLabelShort(value) {
  if (value === "responses") return "Responses";
  if (value === "anthropic_messages") return "Messages";
  return "Chat";
}
...
function startCustomModelEdit(presetId, options = {}) {
  ...
  setValue(
    "#customApiType",
    model.api === "responses" || model.api === "anthropic_messages"
      ? model.api
      : "chat_completions",
  );
...
function customModelFormPayload(editingModel = null) {
  const customProviderName = value("#customProviderName").trim();
  const customBaseUrl = value("#customBaseUrl").trim();
  const scopedProvider = scopedCustomProviderId ? providerFor(scopedCustomProviderId) : null;

  const isInheritingScoped = Boolean(
    scopedProvider &&
    !editingModel &&
    (!customProviderName || customProviderName === scopedProvider.name || customProviderName === scopedProvider.id) &&
    (!customBaseUrl || customBaseUrl === scopedProvider.baseUrl)
  );

  if (isInheritingScoped) {
    return customModelFromProvider(scopedProvider);
  }

  const providerName = customProviderName || editingModel?.providerName || "Custom";
  ...
  return {
    ...
    providerId,
    providerName,
    displayName: value("#customDisplayName") || value("#customModelName") || "Custom Model",
    model: value("#customModelName"),
    baseUrl: customBaseUrl || editingModel?.baseUrl || "",
...
function customProviderPayload(includeApiKey = false) {
  ...
  if (isInheritingScoped) {
    return {
      ...
      id: scopedProvider.id,
      name: scopedProvider.name || "",
      shortName: scopedProvider.shortName || scopedProvider.name || "",
      baseUrl: scopedProvider.baseUrl || "",
      api: value("#customApiType") || providerApiValue(scopedProvider),
      keyEnv: scopedProvider.keyEnv || scopedProvider.apiKeyEnv || "",
      keyUrl: scopedProvider.keyUrl || "",
      docsUrl: scopedProvider.docsUrl || "",
      logoUrl: scopedProvider.logoUrl || "",
      authMode: scopedProvider.authMode || "api_key",
      custom: true,
      apiKey: "",
    };
  }
