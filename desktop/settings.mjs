...
export function listCodexResources({
  ...
  const chatGptPluginPageApps = appServerSnapshot?.apps?.ok === true
    ...
    : [];
  const effectiveAppServerSkills = appServerSnapshot?.skills?.ok === true
    ? appServerSnapshot.skills
    : scanDiskSkillsFallback(homeDir);
  const chatGptPluginPageSkillSnapshot = normalizeCodexPluginPageSkills(effectiveAppServerSkills.items, { homeDir });
  ...
  const chatGptPluginPageReadStatus = {
    ...
    apps: appServerSnapshot?.apps?.ok === true
      ...
      : { ok: true, source: "disk-scan", label: "已从磁盘读取" },
    mcpServers: { ok: true, source: "disk-scan", label: "已从磁盘读取" },
    skills: { ok: true, source: "disk-scan", label: "已从磁盘读取" },
  };
...
export function readCodexAppServerResourceSnapshot({
  ...
  if (!executable) {
    return {
      executable: "",
      ok: true,
      plugins: { ok: false, items: [] },
      apps: { ok: false, items: [] },
      skills: scanDiskSkillsFallback(homeDir),
      desktopPluginPagePolicy,
      code: "ok_disk_fallback",
    };
  }
  ...
  try {
    ...
    const freshValue = {
      ...
      ...parsed,
      skills: parsed?.skills?.ok === true ? parsed.skills : scanDiskSkillsFallback(homeDir),
      desktopPluginPagePolicy,
    };
...
function scanDiskSkillsFallback(homeDir = os.homedir()) {
  const skillsDir = path.join(homeDir, ".codex", "skills");
  if (!fs.existsSync(skillsDir)) {
    return { ok: true, items: [], code: "ok" };
  }
  const items = [];
  try {
  ...
  return { ok: true, items, code: "ok_disk_fallback" };
}
...
export function providerCatalog(rootDir, state = null) {
  ...
  for (const model of customModels) {
    ...
    if (!customProviders.has(model.providerId)) {
      ...
      const name = override.name || override.shortName || model.providerName || model.providerId;
      const shortName = override.shortName || override.name || model.providerName || "Custom";
      customProviders.set(model.providerId, {
        id: model.providerId,
        name,
        shortName,
        keyEnv: override.keyEnv || model.keyEnv,
        keyLabel: `${name} API Key`,
        keyUrl: override.keyUrl || model.keyUrl || "",
        docsUrl: override.docsUrl || model.docsUrl || "",
        baseUrl: override.baseUrl || model.baseUrl,
        api: override.api || model.api || "chat_completions",
        logoUrl: override.logoUrl || model.logoUrl || "",
        authMode: override.authMode || model.authMode || "api_key",
...
function applyProviderSettingsToModel(model, provider) {
  ...
  const currentProviderName = provider.shortName || provider.name || provider.id;
  let displayName = model.displayName || model.model || "";
  if (model.providerName && model.providerName !== currentProviderName && displayName.startsWith(model.providerName)) {
    displayName = currentProviderName + displayName.slice(model.providerName.length);
  }
  const next = {
    ...model,
...
function applyProviderOverride(provider, override) {
  ...
  const name = override.name || provider.name;
  const shortName = override.name || override.shortName || provider.shortName || provider.name;
  return {
    ...provider,
    ...override,
    name,
...
function providerDirectoryModelApi(provider = {}, entry = {}, remoteModel = {}, exact = null, fallback = null) {
  ...
  // Detect Claude only from the model identity itself.
  // Do not consult fallback templates, or one Claude seed model would
  // pollute every other remote model under the same provider.
  const modelBlob = [
    remoteModel.id,
    ...
    .join(" ");
  const looksClaude =
    modelBlob.includes("claude") ||
    modelBlob.includes("anthropic") ||
    /(^|[^a-z0-9])(sonnet|opus|haiku)([^a-z0-9]|$)/.test(modelBlob);
  if (looksClaude) {
    return "anthropic_messages";
  }
  // Prefer exact model api, then provider api. Never inherit anthropic from fallback.
  if (exact?.api === "responses" || exact?.api === "chat_completions" || exact?.api === "anthropic_messages") {
    return exact.api === "anthropic_messages" ? "chat_completions" : exact.api;
  }
...
function mutateConfigState(rootDir, baseState, operation, payload = {}, options = {}) {
  ...
  if (operation === "configPackage:import" || operation === "configPackage:restoreLatestImportBackup") {
    ...
    const saved = normalizeProviderOverride({
      ...
      ...input,
      shortName: input.shortName || input.name || input.displayName,
      id: providerId,
      updatedAt: configMutationNow(options),
    });
    if (!saved) {
    ...
    touched.add("providerOverrides");
    const newName = saved.shortName || saved.name || "";
    if (newName) {
      for (const item of state.customModels) {
        if (item.providerId === providerId) {
          const oldName = item.providerName;
          item.providerName = newName;
          if (oldName && item.displayName && item.displayName.startsWith(oldName)) {
            item.displayName = newName + item.displayName.slice(oldName.length);
          }
        }
      }
      touched.add("customModels");
  ...
  } else if (operation === "providers:remove") {
    const providerId = String(payload.providerId || payload || "").trim();
    if (!providerId) {
      throw new Error("Provider id is required.");
    }
    const providerName = providerCatalog(rootDir, state)
      .find((provider) => provider.id === providerId)?.name || providerId;
    const removedModelPresetIds = new Set(
      state.customModels
        .filter((model) => model.providerId === providerId)
        .map((model) => model.presetId)
    );
    state.customModels = state.customModels.filter((model) => model.providerId !== providerId);
    state.selectedModelIds = state.selectedModelIds.filter((id) => !removedModelPresetIds.has(id));
    delete state.providerOverrides[providerId];
    touched.add("customModels");
    touched.add("providerOverrides");
    operationResult = { providerId, providerName };
    ...
    if (model.providerId && model.providerName) {
      const newName = model.providerName;
      for (const item of state.customModels) {
        if (item.providerId === model.providerId) {
          const oldName = item.providerName;
          item.providerName = newName;
          if (oldName && item.displayName && item.displayName.startsWith(oldName)) {
            item.displayName = newName + item.displayName.slice(oldName.length);
      ...
      if (state.providerOverrides[model.providerId]) {
        state.providerOverrides[model.providerId].name = newName;
        state.providerOverrides[model.providerId].shortName = newName;
        touched.add("providerOverrides");
      }
    }
...
function routeForSelectedModel(
  ...
  if (
    (route.api === "chat_completions" || route.api === "anthropic_messages") &&
    route.maxToolContinuationTurns === undefined
  ) {
    route.maxToolContinuationTurns =
      route.api === "anthropic_messages"
        ? Math.min(DEFAULT_CHAT_TOOL_CONTINUATION_TURNS, 2)
        : DEFAULT_CHAT_TOOL_CONTINUATION_TURNS;
  }
  if (route.api === "anthropic_messages") {
    route.preferClaudeMessages = true;
  }
...
function slugify(value) {
  ...
  const base = ascii ? `${ascii.slice(0, 24)}-` : "";
  return `${base}${hex}`;
}
...
function slugifyEnv(value) {
  ...
  const base = ascii ? `${ascii.slice(0, 24)}_` : "";
  return `${base}${hex}`;
}
