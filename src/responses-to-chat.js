...
const MAX_CHAT_DATA_IMAGE_URL_CHARS = 8_000_000;
const MAX_CHAT_DATA_IMAGE_BYTES = 750_000;
const MAX_CHAT_IMAGE_SIDE = 1600;
const CHAT_IMAGE_JPEG_QUALITY = 78;
const MAX_CHAT_IMAGE_MESSAGES = 4;
...
export function responseRequestToChatSourceMessages(request, route, history, options = {}) {
  ...
  const priorMessages = persistedPriorMessages;
  ...
  const historySourceMessages = sanitizeMessagesForRoute(
    stripAllButRecentImageMessageData(normalizeToolCallPairs([
      ...headerMessages,
      ...priorMessages,
      ...currentMessages,
    ], {
  ...
  if (contextSwitchSummary && priorMessages.length > 0) {
    const protectedMessages = Array.isArray(options.contextSwitchCompaction?.protectedMessages)
      ...
      : [];
    const compactedSourceMessages = sanitizeMessagesForRoute(
      stripAllButRecentImageMessageData(normalizeToolCallPairs([
        ...headerMessages,
...
function stripAllButRecentImageMessageData(messages) {
  const source = Array.isArray(messages) ? messages : [];
  const retainedImageMessageIndexes = new Set(
    source
      .map((message, index) =>
        contentHasDataImage(message?.content) ? index : -1,
      )
      .filter((index) => index >= 0)
      .slice(-MAX_CHAT_IMAGE_MESSAGES),
  );
  return source.map((message, index) =>
    !retainedImageMessageIndexes.has(index)
      ? {
          ...message,
          content: stripHistoricalImageDataFromContent(message?.content),
        }
      : message,
  );
}
...
function contentHasDataImage(content) {
  ...
  return parts.some((part) => {
    if (!part || typeof part !== "object" || !isImagePart(part)) {
      return false;
    }
...
function stripHistoricalImageDataFromContent(content) {
  if (!Array.isArray(content)) {
    return stripHistoricalImageDataFromPart(content);
  }
  return content.map(stripHistoricalImageDataFromPart);
}

function stripHistoricalImageDataFromPart(part) {
  if (!part || typeof part !== "object" || !isImagePart(part)) {
    return part;
  }
  const rawImageUrl = part.image_url ?? part.imageUrl ?? part.url;
  ...
  return { type: "input_text", text: HISTORICAL_IMAGE_PLACEHOLDER };
}
...
export function responseInputToChatMessages(input, toolContext, route = {}) {
  ...
  for (const item of items) {
    ...
    if (isResponseToolOutputItem(item)) {
      const toolResultImage = imageDataUrlFromToolOutput(item);
      const toolMessage = chatMessageFromToolOutput(item);
      messages.push(
        toolResultImage
          ? toolMessageWithoutDataImages(toolMessage)
          : toolMessage,
      );
      if (toolResultImage && shouldForwardImagesToChat(route)) {
        messages.push({
          role: "user",
          content: [
            {
              type: "image_url",
              image_url: { url: normalizeDataImageForChat(toolResultImage) },
            },
          ],
        });
      }
...
function imagePartToChat(part, route = {}) {
  ...
  const imageUrl = { url: normalizeDataImageForChat(url) };
...
export function normalizeDataImageForChat(value) {
  if (!shouldTranscodeDataImage(value) || process.platform !== "win32") {
    return value;
  }
  let directory = "";
  try {
    const source = Buffer.from(value.slice(value.indexOf(",") + 1), "base64");
    directory = mkdtempSync(join(tmpdir(), "codexbridge-image-"));
    const inputPath = join(directory, "source");
    writeFileSync(inputPath, source);
    const base64 = execFileSync(
      "powershell.exe",
      ["-NoProfile", "-NonInteractive", "-Command", windowsImageTranscodeScript()],
      {
        encoding: "utf8",
        timeout: 15_000,
        windowsHide: true,
        maxBuffer: MAX_CHAT_DATA_IMAGE_BYTES * 2,
        env: { ...process.env, CODEXBRIDGE_IMAGE_INPUT_PATH: inputPath },
      },
    ).trim();
    return base64 ? `data:image/jpeg;base64,${base64}` : value;
  } catch {
    return value;
  } finally {
    if (directory) {
...
function imageDataUrlFromToolOutput(item) {
  const output = stringifyJson(item?.output ?? item?.result ?? "");
  return output.match(/data:image\/[a-z0-9.+-]+;base64,[a-z0-9+/=]+/i)?.[0] || "";
}

function toolMessageWithoutDataImages(message) {
...
function shouldTranscodeDataImage(value) {
  return (
    isLargeDataImageUrl(value) ||
    (typeof value === "string" && /^data:image\/webp[;,]/i.test(value))
  );
}

function isLargeDataImageUrl(value) {
  if (typeof value !== "string" || !/^data:image\//i.test(value)) {
    return false;
  }
  const payloadStart = value.indexOf(",");
  return payloadStart >= 0 && Math.floor(((value.length - payloadStart - 1) * 3) / 4) > MAX_CHAT_DATA_IMAGE_BYTES;
}

function windowsImageTranscodeScript() {
  return [
    "$ErrorActionPreference = 'Stop'",
    "Add-Type -AssemblyName PresentationCore",
    "$bytes = [System.IO.File]::ReadAllBytes($env:CODEXBRIDGE_IMAGE_INPUT_PATH)",
    "$input = New-Object System.IO.MemoryStream",
    "$input.Write($bytes, 0, $bytes.Length)",
    "$input.Position = 0",
    "$decoder = [System.Windows.Media.Imaging.BitmapDecoder]::Create($input, [System.Windows.Media.Imaging.BitmapCreateOptions]::PreservePixelFormat, [System.Windows.Media.Imaging.BitmapCacheOption]::OnLoad)",
    "$source = $decoder.Frames[0]",
    `$scale = [Math]::Min(1.0, ${MAX_CHAT_IMAGE_SIDE} / [Math]::Max($source.PixelWidth, $source.PixelHeight))`,
    "$bitmap = $source",
    "if ($scale -lt 1.0) { $scaled = New-Object System.Windows.Media.Imaging.TransformedBitmap; $scaled.BeginInit(); $scaled.Source = $source; $scaled.Transform = New-Object System.Windows.Media.ScaleTransform($scale, $scale); $scaled.EndInit(); $bitmap = $scaled }",
    "$encoder = New-Object System.Windows.Media.Imaging.JpegBitmapEncoder",
    `$encoder.QualityLevel = ${CHAT_IMAGE_JPEG_QUALITY}`,
...
export function attachmentGuidanceFromRequest(request = {}) {
...
export function systemInstructionsFromRequest(request) {
  const parts = [];
...
export function toolGuidanceFromContext(toolContext, request = {}) {
...
export function responseToolsForChatRequest(request = {}, options = {}) {
...
function routeSupportsReasoningContent(route = {}) {
  // Claude-compatible chat gateways usually reject reasoning_content fields.
  if (isClaudeCompatibleRoute(route)) {
    return false;
  }
...
function shouldFlattenToolCallHistory(route = {}) {
  ...
  // Many Claude OpenAI-compatible gateways reject long native tool transcripts.
  if (isClaudeCompatibleRoute(route)) {
    return true;
  }
...
function isClaudeCompatibleRoute(route = {}) {
  const blob = [
    route.provider,
    route.providerId,
