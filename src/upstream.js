...
import {
  ...
  chatPayloadToClaudeMessages,
  claudeHistoryWithoutImages,
  claudeMessageToResponse,
  claudeMessagesHeaders,
  claudeMessagesUrlForRoute,
  isClaudeLikeRoute as isClaudeLikeRouteShared,
  readClaudeMessagesMessage,
  readClaudeMessagesAsChatCompletion,
  responsesToClaudeMessagesRequest,
  shouldUseClaudeMessagesUpstream,
...
export class UpstreamHttpError extends Error {
  constructor(statusCode, bodyText, upstreamUrl, route = {}, options = {}) {
    const snippet = summarizeUpstreamBody(bodyText);
    super(
      snippet
        ? `Upstream returned HTTP ${statusCode}: ${snippet}`
...
function summarizeUpstreamBody(bodyText, limit = 240) {
  ...
  try {
    ...
    const text = [type, message].filter(Boolean).join(": ") || raw;
...
export async function handleResponsesRequest(
  ...
  try {
    ...
    if (compactKind && (route.api === "chat_completions" || route.api === "anthropic_messages")) {
    ...
    if (route.api === "anthropic_messages") {
      return await proxyAnthropicMessages(requestBody, route, history, res, requestContext);
    }
...
export async function proxyAnthropicMessages(
  ...
  const nativeRoute = { ...route, api: "anthropic_messages", preferClaudeMessages: true };
  const converted = responsesToClaudeMessagesRequest(requestBody, nativeRoute, history, {
    capabilityProviders: context.capabilityProviders,
    contextSwitchCompaction: context.contextSwitchCompaction,
  });
  logToolDiagnostics(context, nativeRoute, converted.toolDiagnostics, "anthropic-native");
  const upstreamUrl = claudeMessagesUrlForRoute(nativeRoute, joinUpstreamUrl);
  logRoute(context, nativeRoute, upstreamUrl);
  let messagesForHistory = converted.messagesForHistory;
  let message;
  try {
    message = await callNativeClaudeMessages(
      upstreamUrl,
      nativeRoute,
      converted.body,
      context,
    ...
    if (!shouldRetryClaudeWithoutImages(error, converted.body)) {
      throw error;
    }
    console.warn(
      `[${new Date().toISOString()}] ${context.requestId || "req"} ` +
        `!! anthropic-native route=${nativeRoute.id} image rejected; retrying without images`,
    );
    messagesForHistory = claudeHistoryWithoutImages(messagesForHistory);
    try {
      message = await callNativeClaudeMessages(
        upstreamUrl,
        nativeRoute,
        claudeBodyWithoutImages(converted.body),
        context,
        { trackRateLimit: false },
      );
    } catch (retryError) {
      return sendLocalClaudeImageRejectedResponse({
        requestBody,
        route: nativeRoute,
        history,
        res,
  ...
  let response = claudeMessageToResponse(
    message,
    requestBody.model || nativeRoute.id,
  ...
  recordHistoryTurn(
    history,
    response,
    [...messagesForHistory, assistantHistoryMessageFromClaude(message)],
    {
      api: "anthropic_messages",
      routeId: nativeRoute.id || "",
      upstreamModel: nativeRoute.model || "",
      upstreamKnown: true,
      ...responseRequestUserMeta(requestBody),
      toolContinuationTurns: responseHasRunnableToolCall(response) ? toolContinuationTurns : 0,
      noProgressToolLoopTurns: responseHasRunnableToolCall(response)
        ? noProgressToolLoopTurns
        : 0,
      toolCallSignatures: responseHasRunnableToolCall(response) ? toolCallSignatures : [],
      toolResultSignatures,
    },
    { requestBody, route: nativeRoute },
  );
...
export async function proxyChatCompletions(
  ...
  let response = chatResponseToResponse(
    ...
    converted.toolContext,
    {
      ...
      preferVisibleToolStatus: isClaudeLikeRoute(route),
  ...
  recordHistoryTurn(
    ...
    {
      api: route.api === "anthropic_messages" ? "anthropic_messages" : "chat_completions",
      routeId: route.id || "",
...
async function callChatCompletionsUpstream(
  ...
  // Claude relays (e.g. baiyuan) often keep healthy Anthropic /v1/messages pools
  // while OpenAI /v1/chat/completions returns 503 "No available accounts".
  if (shouldUseClaudeMessagesUpstream(route) && options.forceChatCompletions !== true) {
    try {
      return await callClaudeMessagesAsChatCompletions(route, payload, context, options);
    } catch (error) {
      // Official anthropic_messages routes should not silently degrade to the broken chat pool.
      if (route?.api === "anthropic_messages" || !shouldFallbackClaudeMessagesToChat(error)) {
        throw error;
      }
      console.warn(
        `[${new Date().toISOString()}] ${context.requestId || "req"} ` +
          `!! claude-messages route=${route.id} failed; fallback chat_completions ` +
          `status=${error?.statusCode || "n/a"} detail=${safeText(error?.message || String(error), 180)}`,
      );
    }
  }
  ...
  try {
    ...
    if (
      shouldUseClaudeMessagesUpstream(route) &&
      isNoAvailableAccountsError(error) &&
      options.forceChatCompletions !== true
    ) {
      console.warn(
        `[${new Date().toISOString()}] ${context.requestId || "req"} ` +
          `!! chat_completions route=${route.id} no-available-accounts; retrying anthropic messages`,
      );
      return callClaudeMessagesAsChatCompletions(route, payload, context, options);
    }
...
async function callClaudeMessagesAsChatCompletions(route, payload, context = {}, options = {}) {
  const messagesUrl = claudeMessagesUrlForRoute(route, joinUpstreamUrl);
  logRoute(context, route, messagesUrl);
  const messagesPayload = chatPayloadToClaudeMessages(
    {
      ...payload,
  ...
  // Baiyuan-like Claude pools are reliable on streaming Anthropic messages.
  messagesPayload.stream = true;
  ...
  try {
    upstream = await fetchUpstream(
      ...
      {
        method: "POST",
        headers: claudeMessagesHeaders(route, context, requireApiKey, {
          acceptEventStream: true,
        }),
  ...
  return readClaudeMessagesResponse(upstream, messagesUrl, route, context);
}

async function callNativeClaudeMessages(
  messagesUrl,
  route,
  payload,
  context = {},
  ...
  if (!upstream.ok) {
    ...
    throw new UpstreamHttpError(upstream.status, text, messagesUrl, route, {
      headers: upstream.headers,
    });
  }

  try {
    return await readClaudeMessagesMessage(upstream, context);
  } catch (error) {
    ...
    throw new UpstreamHttpError(
      502,
      `Claude messages parse failed: ${safeText(error?.message || String(error), 400)}`,
      messagesUrl,
      route,
    );
  }
...
async function readClaudeMessagesResponse(upstream, upstreamUrl, route, context = {}) {
  if (!upstream.ok) {
    const text = await readUpstreamText(upstream, context);
  ...
  try {
    return await readClaudeMessagesAsChatCompletion(upstream, context);
  } catch (error) {
    if (error instanceof UpstreamHttpError) {
      throw error;
    }
...
function shouldFallbackClaudeMessagesToChat(error) {
  if (!(error instanceof UpstreamHttpError)) {
    return true;
  }
...
function isNoAvailableAccountsError(error) {
  const blob = `${error?.message || ""} ${error?.bodyText || ""}`.toLowerCase();
  return blob.includes("no available accounts") || blob.includes("no available account");
}
...
function maxChatToolContinuationTurns(route = {}) {
  ...
  if (Number.isFinite(value) && value >= 0) {
    turns = Math.floor(value);
  } else {
    turns = DEFAULT_CHAT_TOOL_CONTINUATION_TURNS;
  }
  // Claude-compatible OpenAI proxies degrade badly on long tool loops.
  if (isClaudeLikeRoute(route)) {
    return Math.min(turns, 2);
  }
  return turns;
}

function isClaudeLikeRoute(route = {}) {
  return isClaudeLikeRouteShared(route);
}
...
function sendLocalClaudeImageRejectedResponse({
  requestBody,
  route,
  history,
  res,
...
function shouldRetryClaudeWithoutImages(error, body) {
  if (!(error instanceof UpstreamHttpError) || !claudeBodyHasImages(body)) {
    return false;
  }
  const statusCode = Number(error.statusCode);
  if (![400, 415, 422].includes(statusCode)) {
  ...
  return (
    !upstreamText ||
    /image|vision|multi[-\s]?modal|unsupported media|content|part|invalid request/.test(
      upstreamText,
    )
  );
}

function claudeBodyHasImages(body) {
  return Array.isArray(body?.messages) && body.messages.some((message) =>
    Array.isArray(message?.content) && message.content.some((part) => part?.type === "image"),
  );
}

function claudeBodyWithoutImages(body) {
  const sanitized = cloneJson(body);
  sanitized.messages = Array.isArray(sanitized.messages)
    ? sanitized.messages.map((message) => ({
        ...message,
        content: Array.isArray(message?.content)
          ? message.content.map((part) =>
              part?.type === "image"
                ? { type: "text", text: IMAGE_REJECTED_PLACEHOLDER }
                : part,
            )
          : message?.content,
      }))
...
export async function callJsonUpstream(
  ...
  logUpstreamPayloadStats(context, route, serializedPayload);
  let upstream;
  try {
    upstream = await fetchUpstream(upstreamUrl, {
      method: "POST",
...
function logUpstreamPayloadStats(context, route, serializedPayload) {
  const imageLengths = [...String(serializedPayload || "").matchAll(/data:image\/[^;,]+;base64,([A-Za-z0-9+/=]+)/g)]
    .map((match) => match[0].length);
  console.log(
    `[${new Date().toISOString()}] ${context.requestId || "req"} upstream_payload ` +
      `route=${route.id || route.model || "unknown"} bytes=${Buffer.byteLength(serializedPayload)} ` +
      `data_images=${imageLengths.length} image_chars=${imageLengths.join(",") || "-"}`,
  );
}
...
function upstreamHeaders(route, context = {}, options = {}) {
  const headers = {
    ...
    "user-agent": "CodexBridge/0.3.13 (OpenAI-Compatible Client)",
...
async function fetchUpstream(upstreamUrl, init, context = {}, route = {}, options = {}) {
  ...
  let response;
  try {
    response = await fetchAndTrackRateLimit(upstreamUrl, proxiedInit, route, proxiedOptions, context);
  } catch (initialError) {
    ...
    if (
      ...
      if (refreshedInit.dispatcher && refreshedInit.dispatcher !== proxiedInit.dispatcher) {
        ...
        try {
          response = await fetchAndTrackRateLimit(
            upstreamUrl,
            refreshedInit,
            route,
            proxiedOptions,
    ...
    if (!response && usedProxy && isNetworkFetchFailure(error)) {
    ...
    if (!response) {
      ...
      try {
        response = await fetchAndTrackRateLimit(upstreamUrl, init, route, options, context);
      } catch (directError) {
  ...
  if (response && [502, 503, 504].includes(response.status) && !options._is50xRetry) {
    ...
    try {
      const retryResponse = await fetchAndTrackRateLimit(
        ...
        proxiedInit,
        route,
        { ...proxiedOptions, _is50xRetry: true },
        context,
      );
      if (retryResponse) {
      ...
      // Return initial 502/503 response if retry fails with exception
    }
  }
