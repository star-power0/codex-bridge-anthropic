...
export function chatResponseToResponse(chat, requestedModel, toolContext, options = {}) {
  ...
  // Claude/OpenAI-compatible proxies often bill while returning only tool_calls.
  // Keep tools, but avoid a fully blank Responses payload for Codex UI.
  if (!text && hasRunnableToolCall && options.preferVisibleToolStatus) {
    text = `正在调用 ${toolCalls.length} 个工具…`;
  }
...
function messageText(message) {
  ...
  // Some Claude-compatible gateways put final text outside content.
