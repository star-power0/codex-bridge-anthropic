  ...
  return {
    ...
    maxToolContinuationTurns: positiveInteger(
      ...
      api === "chat_completions" || api === "anthropic_messages"
        ? (isClaudeRoute(route, providerFamily) || api === "anthropic_messages"
          ? Math.min(DEFAULT_CHAT_TOOL_TURNS, 2)
...
export function adapterIdForRoute(route = {}) {
  ...
  if (providerFamily === "claude") return "chat-claude";
...
function applyRouteSpecificPayloadDefaults(payload, profile, dropped) {
  if (
     (profile.api !== "chat_completions" && profile.api !== "anthropic_messages") ||
    payload.stream !== true ||
...
export function reasoningParamsForAdapter(request = {}, route = {}, options = {}) {
  const profile = normalizeAdapterProfile(route);
  if ( (profile.api !== "chat_completions" && profile.api !== "anthropic_messages") || !hasReasoningControls(request)) {
    return {};
  }
...
function normalizedDropParams(route, context = {}) {
  ...
  const unsupportedReasoningParams = (context.api === "chat_completions" || context.api === "anthropic_messages")
    ? CHAT_REASONING_PARAMS.filter((param) => !allowedReasoningParams.has(param))
    : [];
  const claudeDrops =
    (context.api === "chat_completions" || context.api === "anthropic_messages") && isClaudeRoute(route, context.providerFamily)
      ? ["response_format", "parallel_tool_calls", "logit_bias", "n", "user"]
      : [];
  return [...new Set([...configured, ...unsupportedReasoningParams, ...claudeDrops])].sort();
}

function isClaudeRoute(route = {}, providerFamily = "") {
  const family = String(providerFamily || route.providerFamily || "").toLowerCase();
  ...
  const blob = [
    ...
    .join(" ");
  return blob.includes("claude") || blob.includes("anthropic") || blob.includes("sonnet") || blob.includes("opus") || blob.includes("haiku");
}
